...
contract DepositTest is BaseTest {
    ...
    function testDepositedMint(uint256 amount, uint256 lzybra_mint) public {
        ...
        Lzybravault.deposit(amount/2, address(vault),lzybra_mint);

        vm.prank(self);
        Lzybravault.deposit(amount, address(vault),lzybra_mint); // deposit half the amount
         
         

        vm.prank(investor);
        vm.expectRevert(bytes("collateralRatio is Below safeCollateralRatio"));
        Lzybravault.mintLZYBRA(10*10**18,address(vault));

        vm.prank(self);
        Lzybravault.mintLZYBRA(110*10**18,address(vault));
