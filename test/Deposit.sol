...
contract DepositTest is BaseTest {
    ...
    function setUp() public override {
        // Call parent setUp first to initialize base contracts
        super.setUp();

        // Initialize price feed IDs
        ...
        lzybravault.setAssetOracles(address(asset1), address(0), id1);
        lzybravault.setAssetOracles(address(asset2), address(0), id2);
        // Initialize mock price feeds
    }
    function testingDeposit() public {
        ...
        uint256 mintID1 = asset1.requestMint(AMOUNT);
        asset1.approveMint(mintID1, "Test contract mint");

        // Mint for investor
        uint256 mintID2 = asset1.requestMint(AMOUNT, investor);
        asset1.approveMint(mintID2, "Investor mint");

        // Mint for user
        uint256 mintID3 = asset1.requestMint(AMOUNT, user);
        asset1.approveMint(mintID3, "User mint");

        // Repeat for asset2
        uint256 mintID4 = asset2.requestMint(AMOUNT);
        asset2.approveMint(mintID4, "Test contract mint");

        uint256 mintID5 = asset2.requestMint(AMOUNT, investor);
        asset2.approveMint(mintID5, "Investor mint");

        uint256 mintID6 = asset2.requestMint(AMOUNT, user);
        asset2.approveMint(mintID6, "User mint");

        // Repeat for asset3
        uint256 mintID7 = asset3.requestMint(AMOUNT);
        asset3.approveMint(mintID7, "Test contract mint");

        uint256 mintID8 = asset3.requestMint(AMOUNT, investor);
        asset3.approveMint(mintID8, "Investor mint");

        uint256 mintID9 = asset3.requestMint(AMOUNT, user);
        asset3.approveMint(mintID9, "User mint");
    ...
    function testDepositFunctionalityWithMultipleAssetsAndOffers() public {
        ...
        vm.startPrank(investor);
        USDC.approve(address(lzybravault), amount);
        lzybravault.deposit(amount, 1, mint_amount, false, 0);
        lzybravault.deposit(amount, withdrawalAsset1, offer1, mint_amount);
        lzybravault.deposit(amount, withdrawalAsset2, offer2, mint_amount);
        lzybravault.deposit(amount, withdrawalAsset3, offer3, mint_amount);
        assertEq(
    ...
    function testDepositAcrossMultipleOffersSimultaneously() public {
        ...
        assertEq(
            lzybra.balanceOf(user),
            amount,
            "Incorrect balance after deposit in offer 1"
        );
        ...
        assertEq(
            lzybra.balanceOf(user),
            amount * 2,
            "Incorrect balance after deposit in offer 2"
        );
        ...
        assertEq(
            lzybra.balanceOf(user),
            amount * 3,
            "Incorrect balance after deposit in offer 3"
        );
    ...
    function testDepositWithMidTransactionPriceFeedUpdate() public {
        ...
        lzybravault.setAssetOracles(address(USDC), address(0), id1);
    ...
    function testMultipleDepositsWithDynamicOfferAdjustments() public {
        ...
        assertEq(
            lzybra.balanceOf(user),
            initialAmount,
            "User balance mismatch after initial deposit"
        );

        // Adjust the offer details
        OfferStruct memory adjustedOffer = createOffer(
            TakingOfferType.BlockOffer,
            12e18
        );
